...
Our host is configured through `systemd-networkd` and name resolution is augmented by `systemd-resolved`. This configuration places us in a great position to use container based services. We'll discuss those at great length in a future post when we get around to setting up and networking various services, including example `nginx`, `postgres`, `nodejs` and `rust` service deployments.
