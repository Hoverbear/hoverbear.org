...
# Use a big key.
ServerKeyBits 4096
# You shouldn't need to try more than a couple times with a key.
MaxAuthTries 6
# We'll use SSH keys.
PasswordAuthentication no
```

We'll enable both `sshd` and `dhcpcd` which will give us networking and remote access when we reboot:
...
With any luck, in a few minutes you'll be able to `ssh` in again.

When you do, if you get a big scary warning from `ssh` that means it worked! You're seeing this warning because the host's key changed (from rescue to Arch!)
