...
```bash
cat <<EOF > /etc/systemd/network/br-internal.netdev
[NetDev]
Name=br-internal
Kind=bridge
...
cat <<EOF > /etc/systemd/network/br-internal.network
[Match]
Name=br-internal
...
cat <<EOF > /var/lib/machines/base/etc/systemd/network/br-internal.network
[Match]
Name=internal
...
[Service]
# Set up.
ExecStartPre=/usr/bin/ip link add internal type veth peer name peer-%i
ExecStartPre=/usr/bin/ip link set peer-%i master br-internal
ExecStartPre=/usr/bin/ip link set peer-%i up
# Do work
ExecStart=
ExecStart=/usr/bin/systemd-nspawn --quiet --keep-unit --boot --link-journal=try-guest --network-veth --network-interface=internal --settings=override --machine=%I
# Tear down.
ExecStopPost=-/usr/bin/ip link delete peer-%i
```

Try issuing `machinectl clone base test` `machinectl poweroff test puppet`, `machinectl start test puppet`, then try running `machinectl shell test` and see if you can `ping puppet`. Unfortunately you can't `ping google.com`! Darnit!

We can solve this by configuring the bridge a bit more.
