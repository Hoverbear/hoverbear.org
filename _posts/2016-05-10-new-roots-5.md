...
[Service]
# Create adapter.
ExecStartPre=/usr/bin/ip link add pup-%i type veth peer name client-%i
# Add endpoint to puppet.
ExecStartPre=/usr/bin/sh -c "machinectl show --property=Leader puppet | cut -d \"=\" -f 2 | /usr/bin/xargs /usr/bin/ip link set client-%i netns"
...
# Tear down.
ExecStopPost=-/usr/bin/ip link delete client-%i
...
Then run `systemctl edit systemd-nspawn@puppet`:
...
At this point we just need to reconfigure our `base` image to handle any `pup-*` interfaces.
...
We can use a tool called `kexec` to quickly (and uncleanly) switch to the new kernel. On my server this takes under 20s compared to rebooting which can take several minutes. It's important to note that this doesn't *replace* a reboot though, as it doesn't make efforts to clean up anything.

Let's talk about setting it up.
