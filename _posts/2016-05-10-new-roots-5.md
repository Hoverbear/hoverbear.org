---
layout: post
title: "New Roots part 5, Starting with Containers"
...
Since we're basing our `puppet` image on our `base` Arch image we can have them share repository caches. Neat! We'll create `/etc/systemd/nspawn/puppet.nspawn` which is the container specific configuration.
...
Now you can run `systemctl start puppet` (this is the agent) and you can see that `/puppet-test` now exists. Finally run `systemctl enable puppet` and `systemctl enable puppetmaster`.
...
First we'll go ahead and create a bridge.

```bash
cat <<EOF > /etc/system/network/bridge-puppet.netdev
[NetDev]
Name=bridge-puppet
Kind=bridge
Description=The bridge used by puppet to communicate.
EOF

cat <<EOF > /etc/system/network/bridge-puppet.network
[Match]
Name=bridge-puppet

[Network]
Description=The bridge puppet uses to communicate.
DHCP=ipv4
EOF
```

Next, since we'll be adding this link to **all** containers, we'll run `systemctl edit systemd-nspawn@` and add this:

```
[Service]
# Set up.
ExecStartPre=/usr/bin/ip link add pupa-%i type veth peer name pupb-%i
ExecStartPre=/usr/bin/ip link set pupb-%i master bridge-puppet
ExecStartPre=/usr/bin/ip link set pupb-%i up
# Do work
ExecStart=
ExecStart=/usr/bin/systemd-nspawn --quiet --keep-unit --boot --link-journal=try-host --network-veth --network-interface=pupa-%i --settings=override --machine=%I
# Tear down.
ExecStopPost=-/usr/bin/ip link delete pupb-%i
```

At this point we just need to reconfigure our `base` image to handle any `pupa-*` interfaces.

```bash
machinectl read-only base false
cat <<EOF > /var/lib/machines/base/etc/systemd/network/pupa.network
[Match]
Name=pupa-*
...
cat <<EOF > /var/lib/machines/puppet/etc/systemd/network/pupa.network
[Match]
Name=pupa-*
