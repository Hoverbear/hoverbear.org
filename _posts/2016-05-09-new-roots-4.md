...
There are a grand number of things we can do at this point to make our time on the machine enjoyable. Take time to evaluate your choice though. This 'root' host is going to be our 'control seat' but it's not going to be doing much more than orcestrating virtualized environments. Once we configure these environments (next post) we'll be using these for any sort of development, deployment, or experiments.
...
On our 'root' we won't be utilizing this, since we will be the only users. Instead we'll just be modifying the system-wide defaults, this way if we end up acting as the `root` user or something we'll still have the same configuration.
...
Launching `tmux` I can create new 'tabs' with `Ctl+b c` (press `Control` and `b`, then let go and press `c`). I can rename them with `Ctl+b ,`. I can split them with `Ctl+|` and `Ctl+-` respectively. You can move between splits with `Ctl+b left` and `Ctl+b right`.
